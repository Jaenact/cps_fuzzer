...
class ResponseAnalyzer:
    ...
    def __init__(self):
        ...
        self.interesting_regexes = [
            ...
            for pattern, tag in self.INTERESTING_PATTERNS
        ]
...
class CrashDetector:
    ...
    def detect(self, 
               ...
               payload_hash: str = '') -> Tuple[CrashInfo, List[str]]:
        """
        크래시 감지 및 분석
        ...
        for pattern, crash_type, severity in critical_errors:
            if pattern in error_lower:
                return CrashInfo(
                    type=crash_type,
                    severity=severity,
                    description=f"Network error: {pattern}",
                    error_message=error
                )
...
# 팩토리 함수
def create_detector(target_host: str, 
                    target_port: int, 
                    protocol: str = 'tcp') -> CrashDetector:
    """크래시 감지기 생성"""

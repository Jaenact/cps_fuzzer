...
INTERESTING_16 = [
    ...
    512, 1000, 1024, 4096, 32767, 65535
]

INTERESTING_32 = [
    -2147483648, -100663046, -32769, -32768, -129, -128, -1,
    0, 1, 127, 128, 255, 256, 512, 1000, 1024, 4096,
    32767, 32768, 65535, 65536, 100663045, 2147483647
]
...
class ByteFlipMutator(BaseMutator):
    ...
    def mutate(self, data: bytearray) -> Tuple[bytearray, Optional[MutationOp]]:
        ...
        return data, MutationOp(
            ...
            size=flip_size,
            old_value=old_val,
            new_value=bytes(data[pos:pos + flip_size])
        )
...
class InterestingValueMutator(BaseMutator):
    ...
    def mutate(self, data: bytearray) -> Tuple[bytearray, Optional[MutationOp]]:
        ...
        return data, MutationOp(
            ...
            size=int_size,
            old_value=old_val,
            new_value=bytes(data[pos:pos + int_size])
        )
...
class RandomByteMutator(BaseMutator):
    ...
    def mutate(self, data: bytearray) -> Tuple[bytearray, Optional[MutationOp]]:
        ...
        return data, MutationOp(
            ...
            size=num_bytes,
            old_value=old_val,
            new_value=bytes(data[pos:pos + num_bytes])
        )
...
class DictionaryMutator(BaseMutator):
    ...
    DEFAULT_DICT = [
        # 매직 바이트
        b'EFUD', b'ipTM', b'HTTP', b'MAN:', b'M-SEARCH',
        # 명령어 인젝션
        b'; id', b'| id', b'`id`', b'$(id)',
        b'; cat /etc/passwd', b'| ls -la',
        b'; nc -e /bin/sh ', b'; wget ',
        # 포맷 스트링
        b'%s%s%s%s', b'%n%n%n%n', b'%x%x%x%x%x%x',
        b'AAAA%08x.%08x.%08x',
        # 경로 순회
        b'../../../etc/passwd',
        b'....//....//....//etc/passwd',
...
class MutationEngine:
    ...
    def mutate(self, data: bytes, 
               protocol_hints: Dict[str, Any] = None) -> Tuple[bytes, List[MutationOp]]:
        """
        데이터 뮤테이션
...
def create_udp_mutator() -> MutationEngine:
    ...
    extra_dict = [
        b'EFUD', b'ipTM',
        b'get_config\x00', b'set_config\x00',
        b'status\x00', b'reboot\x00',
        b'\x00\x01', b'\x00\x02', b'\x00\x03',  # 명령어 코드
    ]
...
def create_http_mutator() -> MutationEngine:
    ...
    extra_dict = [
        ...
        b'HTTP/1.1\r\n', b'HTTP/1.0\r\n',
        b'Host: ', b'Content-Type: ', b'Content-Length: ',
        b'Cookie: ', b'Authorization: ',
        b'/cgi/d.cgi', b'/cgi/firmware.cgi',
        b'/login.cgi', b'/admin/',
...
def create_upnp_mutator() -> MutationEngine:
    ...
    extra_dict = [
        ...
        b'SOAPAction:',
        b'AddPortMapping', b'DeletePortMapping',
        b'GetExternalIPAddress',
        b'urn:schemas-upnp-org:service:WANIPConnection:1',
        b'<s:Envelope', b'</s:Envelope>',
        b'<NewRemoteHost>', b'<NewExternalPort>',
        b'<NewInternalClient>', b'<NewPortMappingDescription>',
    ]
    return MutationEngine(extra_dict=extra_dict)

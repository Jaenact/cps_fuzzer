...
class QEMUSystemHarness:
    ...
    def send_to_guest(self, protocol: str, data: bytes, timeout: float = 5.0) -> Tuple[Optional[bytes], str]:
        """
        게스트 VM으로 데이터 전송
    ...
    def fuzz_one(self, protocol: str, data: bytes, check_alive: bool = True) -> Dict[str, Any]:
        """
        단일 퍼징 실행
        ...
        return {"response": response, "error": error, "crashed": crashed, "timing": timing}
    ...
    def extract(self) -> Optional[Path]:
        ...
        try:
            result = subprocess.run(
                ["binwalk", "-e", "-C", str(self.output_dir), str(self.firmware_path)], capture_output=True, timeout=120
            )
...
# 팩토리 함수
def create_harness(qemu_path: str = None, kernel: str = None, rootfs: str = None, **kwargs) -> QEMUSystemHarness:
    """QEMU 하네스 생성"""
    config = QEMUConfig(
        qemu_system=qemu_path or "qemu-system-mipsel", kernel=kernel or "", rootfs=rootfs or "", **kwargs
    )

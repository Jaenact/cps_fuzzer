...
class BaseFuzzer(ABC):
    ...
    def save_crash(self, result: FuzzResult):
        ...
        # 바이너리 데이터 저장
        crash_file = os.path.join(self.crash_dir, f"crash_{self.name}_{timestamp}_{crash_hash}.bin")
        with open(crash_file, "wb") as f:
            f.write(result.case.data)

        # 메타데이터 저장
        meta_file = os.path.join(self.crash_dir, f"crash_{self.name}_{timestamp}_{crash_hash}.json")

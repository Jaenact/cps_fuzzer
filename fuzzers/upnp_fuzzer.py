...
class UPnPFuzzer(BaseFuzzer):
    ...
    def _make_soap_request(self, action: str, args: dict, service: str = None) -> bytes:
        """SOAP 요청 생성"""
        if service is None:
...
{args_xml}    </u:{action}>
    ...
    def _mutate_soap_action(self, data: bytes) -> bytes:
        ...
        return self._make_soap_request(random.choice(actions), {}, random.choice(self.SERVICE_URNS))

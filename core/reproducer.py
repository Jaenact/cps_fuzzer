...
class TestCaseRecorder:
    ...
    def finish_case(self, 
                    ...
                    crash_type: str = "",
                    interesting: bool = False,
                    tags: List[str] = None) -> ReproducibleTestCase:
        """테스트케이스 기록 완료"""
        self.case_counter += 1
    ...
    def save_case(self, case: ReproducibleTestCase, 
                  include_binary: bool = True) -> Tuple[Path, Optional[Path]]:
        """테스트케이스 저장"""
        # JSON 메타데이터
...
"""
PoC for Test Case: {case.case_id}
...
# Payload (hex decoded)
PAYLOAD = bytes.fromhex("{case.final_data_hex}")

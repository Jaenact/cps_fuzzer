...
class RedisCoordinator:
    ...
    def register_node(self):
        ...
        self.redis.hset(
            self._key(self.KEY_NODES),
            self.node_id,
            json.dumps(asdict(self.node_info))
        )
    ...
    def share_corpus(self, data: bytes, protocol: str = "", 
        ...
        if not self.redis.hexists(self._key(self.KEY_CORPUS), corpus_hash):
            ...
            # 새 코퍼스 알림 발행
            self.redis.publish(
                self._key("events:corpus"),
                json.dumps({'hash': corpus_hash, 'protocol': protocol})
            )
    ...
    def share_crash(self, 
        ...
        self.redis.hset(
            self._key(self.KEY_CRASHES),
            crash_id,
            json.dumps(asdict(crash))
        )
        
        # 크래시 알림 발행
        self.redis.publish(
            self._key("events:crash"),
            json.dumps({
                'crash_id': crash_id,
                'protocol': protocol,
                'severity': severity
    ...
    def get_crashes(self, protocol: str = None, 
                    limit: int = 100) -> List[SharedCrash]:
        """공유된 크래시 가져오기"""
        crashes = []
...
class LocalCoordinator:
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        return {
            'active_nodes': 1,
            'shared_corpus_size': len(self.corpus),
            'shared_crash_count': len(self.crashes)
        }
    
    def update_stats(self, **kwargs): pass
...
# 팩토리 함수
def create_coordinator(redis_url: str = None, 
                       node_id: str = None,
                       namespace: str = "iptime"):
    """코디네이터 생성"""

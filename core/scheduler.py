...
class EnergyScheduler:
    ...
    def __init__(
        self, strategy: ScheduleStrategy = ScheduleStrategy.FAST, min_energy: float = 0.1, max_energy: float = 100.0
    ):
    ...
    def add(
        self, data: bytes, parent_id: Optional[str] = None, protocol: str = "", source: str = "initial"
    ) -> CorpusEntry:
        """
        새 엔트리 추가
        ...
        entry = CorpusEntry(id=entry_id, data=data, depth=depth, parent_id=parent_id, protocol=protocol, source=source)
    ...
    def update_on_result(
        self, entry_id: str, is_crash: bool = False, is_interesting: bool = False, new_coverage: bool = False
    ):
        """
        실행 결과에 따라 엔트리 업데이트
    ...
    def cull(self, keep_top: int = None, min_corpus_size: int = 100) -> int:
        """
        코퍼스 정리 (저에너지 엔트리 제거)

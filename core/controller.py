...
try:
    from monitors.dashboard import WebDashboard, ConsoleDashboard, create_dashboard
    ...
    def setup_fuzzers(self):
        ...
        if dashboard_config.get('enabled', False) and DASHBOARD_AVAILABLE:
            self.dashboard = create_dashboard(
                web=True,
                port=dashboard_config.get('port', 8888)
            )
...
'''
    ...
    fuzzer_group.add_argument('--udp-only', action='store_true', 
                              help='Run UDP fuzzer only')
    fuzzer_group.add_argument('--http-only', action='store_true', 
                              help='Run HTTP fuzzer only')
    fuzzer_group.add_argument('--upnp-only', action='store_true', 
                              help='Run UPnP fuzzer only')
    fuzzer_group.add_argument('--pptp-only', action='store_true', 
                              help='Run PPTP fuzzer only')
    
    # v2.0 options
    parser.add_argument('--dashboard', action='store_true',
                        help='Enable web dashboard (http://127.0.0.1:8888)')
    parser.add_argument('--dashboard-port', type=int, default=8888,
                        help='Dashboard port (default: 8888)')
    parser.add_argument('--redis', type=str, metavar='URL',
                        help='Enable distributed fuzzing with Redis URL')
    parser.add_argument('--pptp', action='store_true',
                        help='Enable PPTP fuzzer (disabled by default)')

...
# v2.0 Optional Imports
try:
    from monitors.dashboard import ConsoleDashboard, WebDashboard, create_dashboard
...
try:
    from core.distributed import create_coordinator
...
class FuzzerController:
    ...
    def setup_fuzzers(self):
        ...
        # PPTP Fuzzer (v2.0)
        if fuzzers_config.get("pptp", {}).get("enabled", False):
            pptp_config = fuzzers_config["pptp"]
            self.fuzzers["pptp"] = PPTPFuzzer(
                {
                    "name": "PPTP",
                    "target_host": target_host,
                    "target_port": pptp_config.get("port", 1723),
                    "timeout": pptp_config.get("timeout", 5.0),
                    "max_iterations": pptp_config.get("max_iterations", 5000),
                    "delay": pptp_config.get("delay", 0.02),
                    "crash_dir": monitoring.get("crash_dir", "./reports/crashes"),
                    "log_dir": monitoring.get("log_dir", "./reports/logs"),
                }
            )

            corpus_dir = corpus.get("pptp_seeds", "./corpus/pptp")
            if os.path.exists(corpus_dir):
                self.fuzzers["pptp"].load_corpus(corpus_dir)

        # v2.0: Dashboard 초기화
        dashboard_config = self.config.get("dashboard", {})
        if dashboard_config.get("enabled", False) and DASHBOARD_AVAILABLE:
            self.dashboard = create_dashboard(web=True, port=dashboard_config.get("port", 8888))

        # v2.0: Distributed 초기화
        dist_config = self.config.get("distributed", {})
        if dist_config.get("enabled", False) and DISTRIBUTED_AVAILABLE:
            redis_url = dist_config.get("redis_url", "")
            if redis_url:
                self.coordinator = create_coordinator(redis_url)
    ...
    def start(self, parallel: bool = True):
        ...
        if parallel:
            ...
            for name, fuzzer in self.fuzzers.items():
                thread = threading.Thread(target=self._run_fuzzer, args=(name, fuzzer), name=f"Fuzzer-{name}")
                thread.daemon = True
                thread.start()
...
"""
    ...
    def _print_final_report(self):
        ...
        for name, fuzzer in self.fuzzers.items():
            ...
            print(
                f"║ {name.upper():8} │ Cases: {stats['total_cases']:>8,} │ "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ipTIME Firmware Fuzzer v2.0",
...
""",
    ...
    parser.add_argument("-n", "--iterations", type=int, default=10000, help="Max iterations per fuzzer")
    parser.add_argument("--sequential", action="store_true", help="Run fuzzers sequentially instead of parallel")

    # Fuzzer selection
    fuzzer_group = parser.add_mutually_exclusive_group()
    fuzzer_group.add_argument("--udp-only", action="store_true", help="Run UDP fuzzer only")
    fuzzer_group.add_argument("--http-only", action="store_true", help="Run HTTP fuzzer only")
    fuzzer_group.add_argument("--upnp-only", action="store_true", help="Run UPnP fuzzer only")
    fuzzer_group.add_argument("--pptp-only", action="store_true", help="Run PPTP fuzzer only")

    # v2.0 options
    parser.add_argument("--dashboard", action="store_true", help="Enable web dashboard (http://127.0.0.1:8888)")
    parser.add_argument("--dashboard-port", type=int, default=8888, help="Dashboard port (default: 8888)")
    parser.add_argument("--redis", type=str, metavar="URL", help="Enable distributed fuzzing with Redis URL")
    parser.add_argument("--pptp", action="store_true", help="Enable PPTP fuzzer (disabled by default)")
    ...
    if args.dashboard:
        ...
        controller.config["dashboard"]["port"] = args.dashboard_port

    # v2.0: Distributed
    if args.redis:
        controller.config["distributed"]["enabled"] = True
        controller.config["distributed"]["redis_url"] = args.redis

    # v2.0: PPTP
    if args.pptp:
        controller.config["fuzzers"]["pptp"]["enabled"] = True

    # Fuzzer selection
    all_fuzzers = ["udp", "http", "upnp", "pptp"]
    if args.udp_only:
        for f in all_fuzzers:
            if f != "udp":
                controller.config["fuzzers"][f]["enabled"] = False
    elif args.http_only:
        for f in all_fuzzers:
            if f != "http":
                controller.config["fuzzers"][f]["enabled"] = False

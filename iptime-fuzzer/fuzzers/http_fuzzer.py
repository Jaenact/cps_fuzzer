...
class HTTPFuzzer(BaseFuzzer):
    ...
    # 흥미로운 파라미터 이름
    PARAMETERS = [
        'username', 'password', 'passwd', 'user', 'pass',
        'cmd', 'command', 'action', 'act', 'do',
        'ip', 'port', 'host', 'addr',
        'mac', 'ssid', 'key', 'psk',
        'file', 'path', 'dir', 'name',
        'url', 'redirect', 'next', 'goto',
        'id', 'idx', 'index', 'num',
    ...
    def _make_post_request(self, path: str, body: str,
    ...
    def _mutate_params_cmd_injection(self, data: bytes) -> bytes:
        ...
        return self._make_post_request(
            endpoint,
            f'{param}={urllib.parse.quote(injection)}'
        )
    ...
    def _mutate_multipart(self, data: bytes) -> bytes:
        ...
        return self._make_multipart_request(
            endpoint,
            {'firmware': random.choice(file_contents)}
        )
